...
router.post("/admin/postagens/nova", async (req, res) => {
  ...
  const erro = [];
  if (categoria == "0") {
    erro.push({ text: "Categoria invalida, registe uma categoria" });
  }
  if (titulo.length < 0){
    erro.push({ text: "Tittle too short" });
  }
  if (!slug) {
    erro.push({ text: "You need write a slug " });
  }
  if (conteudo.length < 0) {
    erro.push({ text: "You need write some words here " });
  } else {
    try{
      const { titulo, descricao, conteudo, slug, categoria } = req.body;
      const post = await Postagens.create({
        titulo, descricao, conteudo, slug, categoria
      });
      await post.save()
