...
router.post("/admin/postagens/nova", (req, res)=>{
  const erros =[]
  const { body={} } = req;
  const { categoria, titulo, slug, conteudo } = body;
  if(categoria == "0"){erros.push({ text: "Categoria invalida, registe uma categoria"})}
  if(titulo.lenght < 0){erros.push({ text: "Tittle too short"})}
  if(slug.lenght < 0){erros.push({ text : "You need write a slug "})}
  if(conteudo.lenght < 0){erros.push({ text : "You need write some words here "})}
})
